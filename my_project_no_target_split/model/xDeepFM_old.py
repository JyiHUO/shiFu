...
class xDeepFM(nn.Module, BaseModel):
    def __init__(self, config):
        ...
        self.emb_layers = nn.ModuleList(
            [nn.Embedding(data_config[key], model_config["CIN"]["D"]) for key in data_config])
    ...
    def forward(self, x):
        ...
        # Input for Linear
        x_21 = t.cat(x_1, 2).squeeze()  # batch * 1 * (m*D)

        # Input for CIN
        x_22 = t.cat(x_1, 1)  # batch * m * D

        # Input for DNN
        x_23 = t.cat([i.squeeze() for i in x_1], 1)  # batch * 1 * (m*D)

        # Output for CIN
        x_32 = self.cin(x_22).squeeze()  # batch * 1 * (H*k)

        # Output for DNN
        x_33 = self.dnn(x_23).squeeze()  # batch * 1 * outdim=100

        x_cat = t.cat([x_21, x_32, x_33], 1)  # batch * 1 * (m*D + H*k + outdim)
