    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            ...
            for i, layer_list in enumerate(layer):
                x = x_list[i]  # different kinds of head
                ...
                # attention op
                k = y.permute(0, 2, 1)  # batch * D * m
                v = y.matmul(k) / np.sqrt(d_k)  # batch * m*m
                z = F.softmax(v, dim=2).matmul(y)  # (batch*m*m x batch*m*D) = batch * m * D
                print(z.size(), x.size())
                z = z + x
