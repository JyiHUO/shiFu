...
class my_dataset(Dataset):
    def __init__(self, t="train"):
        if Config["large_file"]:
            if t == "train":
                self.h5f = h5py.File(Config["hd5_train_path"], 'r')
            elif t == "val":
                self.h5f = h5py.File(Config['hd5_val_path'], 'r')
            elif t == "test":
                self.data = pd.read_csv(Config["test_path"]).values
        else:
            if t == "train":
                self.data = pd.read_csv(Config["train_path"]).values
            elif t == "val":
                self.data = pd.read_csv(Config["val_path"]).values
            elif t == "test":
                self.data = pd.read_csv(Config["test_path"]).values
